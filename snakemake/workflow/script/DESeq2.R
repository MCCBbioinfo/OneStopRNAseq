# For manual contrast for interaction contrast: need to use integer instead of fraction in "contrast" parameter in DESeq2
# Search for "NEED EDIT" and update as needed: 3 places in total

setwd("/Users/kaihu/UMass Medical School Dropbox/KaiHu/MichelleKelliher/Apoorva_Pradhan/04132026_melanoma/results/OneStopRNAseq_exclude_WTnew/interaction/DESeq2") # NEED EDIT
# Rscript -e "rmarkdown::render(         './DESeq2/DESeq2.Rmd',         params=list(             max_fdr=0.05,             min_lfc=0.585,             cookscutoff=FALSE,             indfilter=FALSE,             countFile='../feature_count/TE_included.txt',             annoFile='https://github.com/radio1988/OneStopRNAseq/raw/dev/snakemake/workflow/resources/anno_tables/mm39.ensembl.111.anno.txt',             metaFile='../meta/meta.xlsx',             contrastFile='../meta/contrast.de.xlsx',             blackSamples='sampleXXXXX,sampleYYY'             ),         output_file='DESeq2.html')" > DESeq2/DESeq2.log 2>&1
...
annoFile     <- "https://github.com/radio1988/OneStopRNAseq/raw/dev/snakemake/workflow/resources/anno_tables/mm39.ensembl.111.anno.txt"
metaFile     <- "../meta/meta.xlsx"
contrastFile <- "../meta/contrast.de.xlsx"
blackSamples <- c("sampleXXXXX", "sampleYYY")
...
blackSamplesStr <- gsub(" " , '', blackSamples)
...
# Default plot
maplot <- function(res, anno, anno_col, fname){
  if (anno_col == 1) {
    by_x <- 1
  } else if (anno_col == 2) {
    by_x <- 2
  }
  maplotdat <- merge(anno, data.frame(res), by.x = by_x, by.y=0)
  ggmaplot(maplotdat, 
           main = name,
           genenames = as.vector(maplotdat$Name),
...
process_deseq_res <- function(res="lfcshrink.res", res2="results.res", name='name', anno='anno.df', anno_col = 1, norm_exp="tpm.df", nclass=nclass){
    ## Summary
    print(name)
    ...
    ## For GSEA
    if (anno_col == 1) {
      select_col = c("Name","log2FoldChange_shrunken")
    } else if (anno_col == 2) {
      select_col = c("Gene","log2FoldChange_shrunken")
    }
    rnk <- subset(resdata, select = select_col)
    colnames(rnk) <- c("# Name","log2FoldChange_shrunken")
    rnk <- rnk[order(rnk$log2FoldChange_shrunken), ]
    rnk[, 1] <- toupper(rnk[, 1])
    name_rnk <- gsub("-", ".", name) # to be consistent with Snakemake DE_CONTRAST_NAMES
    write.table(rnk, 
                paste('rnk/', name_rnk, '.rnk', sep = ''), 
    ...
    maplot(res, anno, anno_col, paste0(name, '.maplot.shrunken_lfc.pdf'))
    maplot(res2, anno, anno_col, paste0(name, '.maplot.raw_lfc.pdf'))
    
    volcanoplot(res, anno, anno_col, lfcthresh=min_lfc, sigthresh=max_fdr,
                textcx=.8,  name= paste(name, "LFC_shrunken", sep="."))
    volcanoplot(res2, anno, anno_col, lfcthresh=min_lfc, sigthresh=max_fdr,
                textcx=.8,name= paste(name, "LFC_raw", sep="."))
    
    n1 <- dim(resdata.sig)[2]
    n2 <- dim(norm_exp)[2]
    
    # Use VST normalized count for DEG heatmap:
    resdata2 <- merge(res.df, res2.df, by=0, sort=F, all.x=T)
    resdata2 <- merge(anno, resdata2, by.x=1, by.y=1, sort=F, all.y=T)
    # norm_exp_vst <- vst(dds, blind = FALSE) %>% assay() %>% as.data.frame()
    norm_exp_vst <- tryCatch({
        assay(vst(dds, blind = FALSE))
    }, error = function(e) {
        warning("Standard VST failed, falling back to shift-log (normTransform)")
        assay(normTransform(dds))
    }) %>% as.data.frame()
    
    resdata2 <- merge(resdata2, norm_exp_vst, by.x=1, by.y=0, all.x=T, sort=F)
    sig_idx2 <- resdata2$padj<max_fdr & abs(resdata2$log2FoldChange_shrunken) > min_lfc
    sig_idx2[is.na(sig_idx2)] <- FALSE
    resdata2.sig <- resdata2[sig_idx2,]
    zscore.df <- zscore(resdata2.sig[, (n1-n2+1):n1])
    rownames(zscore.df) <- resdata2.sig[,1]
    Heatmap(zscore.df, nclass = nclass,
            fname = paste(name, "heatmap", sep="."),
            main = paste(name, "LFC >", min_lfc, "FDR <", max_fdr ))
    
    # Legacy codes using DESeq2 normalized count for DEG heatmap, which is not appropriate for visualization compared to VST.
...
df <- df %>%
  # Handle explicit NAs in id_col that cause column_to_rownames failure
  filter(!is.na(.data[[id_col]])) %>%
  filter(.data[[id_col]] != "", .data[[id_col]] != " ") %>%
...
match_salmon_quant_format <- function (df){
  n_abundances <- sum(grepl('abundance', colnames(df)))
  n_counts <- sum(grepl('counts.', colnames(df)))
  n_lengths <- sum(grepl('length.', colnames(df)))
  if (n_abundances + n_counts + n_lengths == ncol(df) & n_abundances == n_counts & n_counts == n_lengths & ncol(df) >= 3){
    return (T)
  }else{
    return (F)
  }
...
} else if (match_salmon_quant_format(df)){
  cat("Salmon Quant CSV format detected\n")
  salmonCountFormat <- T
  df <- df[,grepl('count.', colnames(df))]
  colnames(df) <- gsub("counts\\.", "", colnames(df))
...
# first check col1 (ensemblID), if none in common, check col2 (gene symbol), if still none, warn.
if (sum(anno[, 1] %in% row.names(cts)) >= 1) {
  anno_col <- 1
} else if (sum(anno[, 2] %in% row.names(cts)) >= 1) {
  anno_col <- 2
} else {
...
# }
dds_test <- try(DESeq(dds), silent = TRUE)

if (inherits(dds_test, "try-error")) {
  warning("Standard DESeq failed. Using manual dispersion estimation...\n")
...
for (i in 1:dim(contrast.df)[2]){
  i <- 5 # where the contrast is interaction # NEED EDIT
  # parse 
  ...
  process_deseq_res(res = res, res2=res2, name = name, anno = anno, anno_col = anno_col, norm_exp = normalized_counts, nclass = nclass)
